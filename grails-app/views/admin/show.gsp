	...
	<body>
		...
		<div id="show-admin" class="content scaffold-show" role="main">
			...
			<ol class="property-list admin">
				...
				<g:if test="${adminInstance?.posts}">
				<li class="fieldcontain">
					<span id="posts-label" class="property-label"><g:message code="admin.posts.label" default="Posts" /></span>
				...
				<li class="fieldcontain">
					<span id="user-label" class="property-label"><g:message code="admin.user.label" default="User" /></span>
					
						<span class="property-value" aria-labelledby="user-label"><g:link controller="user" action="show" id="${adminInstance?.user?.id}">${adminInstance?.user?.encodeAsHTML()}</g:link></span>
