	...
	<body>
		<div id="body">
			<div class="block big"><!-- Block Begin -->
				<div class="titlebar">
					<h3>Equipo: <g:fieldValue bean="${teamInstance}" field="name"/></h3>
				...
				<div class="block_cont">
					...
					<ul class="property-list league">
						...
						<g:if test="${teamInstance?.namePlayTeam}">
						<li class="fieldcontain">
							<span id="namePlayTeam-label" class="property-label"><strong>Nombre del Equipo de Juego</strong></span>
							<span class="property-value" aria-labelledby="namePlayTeam-label"><g:fieldValue bean="${teamInstance}" field="namePlayTeam"/></span>
						</li>
						</g:if>
					
						<g:if test="${teamInstance?.players}">
						<li class="fieldcontain">
							<span id="players-label" class="property-label"><strong>Jugadores</strong></span>
								<ol>
								<g:each in="${teamInstance.players}" var="p">
								<!-- span class="property-value" aria-labelledby="players-label"><g:link controller="player" action="show" id="${p.id}">${p?.user.username.encodeAsHTML()}</g:link></span -->
								<li class="property-value" aria-labelledby="players-label">${p?.user.username.encodeAsHTML()}</li>
								</g:each>
								</ol>
						</li>
						</g:if>
